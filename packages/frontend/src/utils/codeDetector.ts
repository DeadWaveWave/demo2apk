/**
 * Code Type Detection Utility (Browser Version)
 * 
 ...
 */
function extractAppNameFromCode(content: string): string | null {
...
export function detectCodeType(content: string): DetectionResult {
  ...
  for (const pattern of HTML_STRUCTURE_PATTERNS) {
    if (pattern.test(trimmed)) {
      htmlScore += 25;
      hints.push('HTML structure detected');
      break;
    }
  }
  ...
  for (const pattern of EMBEDDED_REACT_PATTERNS) {
    if (pattern.test(trimmed)) {
      ...
      hints.push('Embedded React/JSX detected');
      break;
    }
  }
  ...
  for (const pattern of REACT_EXPORT_PATTERNS) {
    if (pattern.test(trimmed)) {
      ...
      hints.push('React export pattern detected');
      break;
    }
  }
  ...
  switch (type) {
    case 'react-component':
      suggestedFilename = isTypeScript ? 'App.tsx' : 'App.jsx';
      break;
    case 'html-react':
    case 'html':
  ...
  const appNameHint = type === 'react-component' ? extractAppNameFromCode(trimmed) : null;
...
/**
 * Get user-friendly label for code type
 */
...
/**
 * Get color class for code type badge
 */
export function getCodeTypeColor(type: CodeType): string {
  switch (type) {
    case 'html':
      return 'text-bp-blue bg-bp-blue/10 border-bp-blue/50';
    case 'html-react':
      return 'text-bp-cyan bg-bp-cyan/10 border-bp-cyan/50';
    case 'react-component':
      return 'text-green-400 bg-green-400/10 border-green-400/50';
    case 'unknown':
      return 'text-bp-dim bg-bp-dim/10 border-bp-dim/50';
  }
}
